...
Tugtainer uses [Apprise](https://github.com/caronc/apprise) for delivery and [Jinja2](https://jinja.palletsprojects.com/) for the message body, so you can target Discord, Telegram, ntfy, email, Gotify, and dozens more.

- A notification is only sent if the rendered body is **non-empty**. With the default template, a run that only contains `available(notified)` results produces an empty body and stays quiet.
- Custom Jinja2 filter `any_worthy` returns true if at least one item resulted in `available`, `updated`, `rolled_back`, or `failed`.

Result values you'll see in the template context:

| Result | Meaning |
|--------|---------|
| `not_available` | No new image found |
| `available` | New image available |
| `available(notified)` | New image, but already in the previous notification |
| `updated` | Container recreated with the new image |
| `rolled_back` | Recreate failed; old image restored |
| `failed` | Recreate failed |
{.dense}

> 
> The default notification template lives in `backend/const.py` in the repo if you ever need to restore it after editing.
{.is-info}

# 7 · Authentication

By default Tugtainer uses **password auth** — the password is stored encrypted in the `/tugtainer` volume. From **v1.6.0** you can swap in an **OpenID Connect** provider (Authentik, Authelia, Keycloak, etc.) via env vars, and optionally disable password auth entirely with `DISABLE_PASSWORD`.

> 
> Auth cookies are not HTTPS-only by default. If you expose Tugtainer beyond localhost, set `HTTPS=true` and `DOMAIN=` and run it behind a reverse proxy with TLS.
{.is-warning}

# 8 · Environment Variables

None are required, but these are the most useful. The full list is in [.env.example](https://github.com/Quenary/tugtainer/blob/main/.env.example).

| Variable | Default | Purpose |
|----------|---------|---------|
| `LOG_LEVEL` | `info` | App log level (`critical`→`trace`) |
| `AGENT_SECRET` | — | Signs backend↔agent requests (mandatory unless unauthenticated agent allowed) |
| `AGENT_ENABLED` | `true` | Disable the built-in agent if this host can't use the socket |
| `DOCKER_HOST` | — | Point at a socket-proxy, e.g. `tcp://socket-proxy:2375` |
| `DISABLE_AUTH` | `false` | Turn off authentication entirely |
| `DISABLE_PASSWORD` | `false` | Disable password login (OIDC-only) |
| `JWT_SECRET_KEY` | autogenerated | Set this so tokens survive a restart |
| `ACCESS_TOKEN_LIFETIME_MIN` | `5` | Access token lifetime |
| `REFRESH_TOKEN_LIFETIME_MIN` | `43200` | Refresh token lifetime (~1 month) |
| `HTTPS` | `false` | Strict cookies — requires HTTPS to sign in |
| `DOMAIN` | — | Restrict auth to one domain |
| `ENABLE_PUBLIC_API` | `false` | Expose unauthenticated public endpoints |
| `GH_TOKEN` | — | GitHub token to avoid rate limits on app self-update checks |
| `OIDC_ENABLED` | `false` | Turn on OpenID Connect auth |
