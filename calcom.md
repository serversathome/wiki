...
1. Change into the directory
    ...
    cd cal.com
    ```
1. Prepare your configuration: Rename `.env.example` to `.env`
    ```bash
    cp .env.example .env
    ```
1. Navigate to Dockge, find the stack named **Cal.com** and replace the compose file with the one below:
